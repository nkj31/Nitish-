...
<div class="row">
<div class="col-lg-6">
<span class="logo">PddLeBeta</span></div>
...
if (@$_GET['q'] == 'quiz' && @$_GET['step'] == 2 && isset($_SESSION['6e447159425d2d']) && $_SESSION['6e447159425d2d'] == "6e447159425d2d" && isset($_GET['endquiz'])== 'end') {
    ...
    $q = mysqli_query($con, "UPDATE history SET status='finished' WHERE username='$_SESSION[username]' AND eid='$_GET[eid]' ") or die('Error197');
        $q = mysqli_query($con, "SELECT * FROM history WHERE eid='$_GET[eid]' AND username='$_SESSION[username]'") or die('Error156');
            ...
            header('location:account.php?q=result&eid=' . $_GET[CURLOPT_USERNAME]);
}
...
<br />';
            ...
            if (strlen($ans) > 0) {
                echo "<font style=\"color:green;font-size:12px;font-weight:bold\">Selected Answer: </font><font style=\"color:#565252;font-size:12px;\">" . $ans . "</font>&nbsp;&nbsp;<a href=update.php?q=quiz&step=2&eid=$eid&n=$sn&t=$total&qid=$qid&delanswer=delanswer><span class=\"\" style=\"font-size:12px;color:darkred\"></span></a><br /><br />";
            }
            ...
            for ($i = 1; $i <= $total; $i++) {
                ...
                echo ';color:black;font-size:16px;font-family:calibri;border-radius:4px">&nbsp;' . $i . '&nbsp;</a>';
            }
        } else {
            unset($_SESSION['6e447159425d2d']);
            $q = mysqli_query($con, "UPDATE history SET status='finished' WHERE username='$_SESSION[username]' AND eid='$_GET[eid]' ") or die('Error197');
...
<a href="#" data-toggle="modal" data-target="#developers" s style="color:lightyellow;" onmouseover="this.style('color:yellow')" target="new"> sharma ji Infotech</a>
</div>
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
		...
		<div class="col-md-4">
		 ...
		 <div class="col-md-5">
		<a href="" style="color:#202020; font-family:'typo' ; font-size:18px" title=""> sharma Ji Infotech </a>
		<h4 style="color:#202020; font-family:'typo' ;font-size:16px" class="title1">+91 9514444471</h4>
		<h4 style="font-family:'typo' "> nitish23ca015@satiengg.in</h4>
		<h4 style="font-family:'typo' "> Samrat Ashok Technologicale Institute Vidisha </h4></div></div>
		</p>
      </div>
